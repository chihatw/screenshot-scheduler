...
### 画像ペアビューアのキーボード操作（拡大表示）

- 画像をクリックすると拡大表示されます。
- さらに、以下のキーボード操作でも拡大表示が可能です：
  - `a` キー：左側の画像（img2）を拡大表示
  - `d` キー：右側の画像（img1）を拡大表示
- 拡大表示中は自動再生が一時停止し、閉じると元の状態に戻ります。
- 拡大表示を閉じるには、画像外の暗い領域をクリック、右上の × ボタンをクリック、または Esc キーを押してください。

---

## 2025/06/02 画像ペアビューアの UI・操作性アップデート

- 画像ペアビューア（index.html）で以下の改善を実施：
...
- これにより、操作ガイドが常に見やすくなり、拡大時の操作性も大幅に向上しました。

---

## process_screenshots.sh の新しい動作仕様（2025/06/04 ～）

- スクリーンショットの「1 日」の区切りを「AM4:00 ～翌日 AM4:00」としています。
  - 例: 6 月 3 日分として扱うのは「6 月 3 日 AM4:00 ～ 6 月 4 日 AM4:00」に撮影された画像です。
...
- スクリプトの実行例:
  ...
  ./scripts/process_screenshots.sh
  ```
- 詳細は scripts/process_screenshots.sh のコメントを参照してください。
